/**
 * Decorador que agrega fresitas a un helado.
 *
 * Esta clase extiende {@code DecoradorHelado} y añade fresitas
 * como topping al helado base. Modifica la descripción del helado
 * y puede afectar su precio.
 *
 * Forma parte del patrón de diseño Decorator, permitiendo agregar
 * características adicionales a un objeto de forma dinámica
 * sin modificar su clase original.
 */
public class DecoradorFresitas extends DecoradorHelado{
    
    /**
     * Construye un decorador de fresitas para el helado dado.
     *
     * @param helado el helado al que se le agregará el decorado.
     */
    public DecoradorFresitas(Helado helado) {
        super(helado);
    }


    /**
     * Regresa el precio del helado con el decorado agregado.
     *
     * @return el precio total del helado.
     */
    @Override
    public double getPrecio() { 
        return helado.getPrecio() + 24.00; 
    }

    /**
     * Regresa la descripción del helado incluyendo el decorado.
     *
     * @return la descripción del helado con fresitas.
     */
    @Override
    public String getDescripcion() { 
        return helado.getDescripcion() + ", fresitas"; 
    }
}